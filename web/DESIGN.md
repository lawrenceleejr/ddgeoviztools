...
| Deploy | GitHub Actions → Pages (`.github/workflows/pages.yml`) | Builds `web/` on push to `main`, uploads `web/dist`. |
...
One-time, in the GitHub repository: Settings → Pages → Source =
**GitHub Actions**. After that every push to `main` publishes. The
existing `blender-render.yml` workflow is untouched.
